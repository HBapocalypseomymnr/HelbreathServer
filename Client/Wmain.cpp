...
{
	PrimaryMonitorInfo* pInfo = (PrimaryMonitorInfo*)dwData;
	MONITORINFO mi;
...
{
	PrimaryMonitorInfo pmInfo = {0, 0, 800, 600, false};
	EnumDisplayMonitors(NULL, NULL, MonitorEnumProc, (LPARAM)&pmInfo);
	if (!pmInfo.found) {
		pmInfo.x = 0;
		pmInfo.y = 0;
		pmInfo.width = GetSystemMetrics(SM_CXSCREEN);
		pmInfo.height = GetSystemMetrics(SM_CYSCREEN);
	}
