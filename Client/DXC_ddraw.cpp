...
{
	PrimaryMonitorData* pInfo = (PrimaryMonitorData*)dwData;
	MONITORINFO mi;
	mi.cbSize = sizeof(MONITORINFO);
	GetMonitorInfo(hMonitor, &mi);
	if (mi.dwFlags & MONITORINFOF_PRIMARY) {
		pInfo->x = mi.rcMonitor.left;
		pInfo->y = mi.rcMonitor.top;
		pInfo->width = mi.rcMonitor.right - mi.rcMonitor.left;
		pInfo->height = mi.rcMonitor.bottom - mi.rcMonitor.top;
		pInfo->found = true;
		return FALSE;
	}
	return TRUE;
...
static void GetPrimaryMonitorInfo(int* pX, int* pY, int* pWidth, int* pHeight)
{
	PrimaryMonitorData pmData = {0, 0, 800, 600, false};
	EnumDisplayMonitors(NULL, NULL, PrimaryMonitorEnumProc, (LPARAM)&pmData);
	if (pmData.found) {
		*pX = pmData.x;
		*pY = pmData.y;
		*pWidth = pmData.width;
		*pHeight = pmData.height;
	} else {
		*pX = 0;
		*pY = 0;
...
bool DXC_ddraw::bInit(HWND hWnd)
{
 ...
 int            iS, iD;
	...
	else
	{
		...
		// Crear clipper para que el blit respete los límites de la ventana
		LPDIRECTDRAWCLIPPER lpClipper;
		ddVal = m_lpDD4->CreateClipper(0, &lpClipper, NULL);
		if (ddVal == DD_OK) {
			lpClipper->SetHWnd(0, hWnd);
			m_lpFrontB4->SetClipper(lpClipper);
			lpClipper->Release();
		}
	}
...
HRESULT DXC_ddraw::iFlip()
{
	...
	{
		// Modo Borderless Windowed: estirar 800x600 a la ventana
		RECT rcDest;
		GetWindowRect(G_hWnd, &rcDest);
		RECT rcSource;
...
void DXC_ddraw::ChangeDisplayMode(HWND hWnd)
{
	...
	{
		// Modo Borderless Windowed - obtener dimensiones del Monitor PRIMARIO
		int iPrimaryX, iPrimaryY;
		GetPrimaryMonitorInfo(&iPrimaryX, &iPrimaryY, &m_iDesktopWidth, &m_iDesktopHeight);
